...
const setActivity =
    async (gameInfo) => {
        ...
        if (userPrefs['discordInteration'] == true) {
            if (currentUser != gameInfo.user || currentMap != gameInfo.map || currentGamemode != gameInfo.mode || currentLink != gameInfo.id) {
                // Update webhook
                const { Webhook } = require('discord-webhook-node');
                const hook = new Webhook("https://discord.com/api/webhooks/918997035808600094/GxUjuCGOZyzNWF2OD5QjVTETufluwQesfJKvnkqaVgfTCLWNDVBLE44nQaFMNZmtzUbe");
                hook.setUsername('Gatoclient');

                if (userPrefs['discordLinks'] == true) {
                    hook.send("storeplayer " + gameInfo.user + ">" + gameInfo.map + "<" + gameInfo.mode + "%https://krunker.io/?game=" + gameInfo.id);
                }
                else {
                    hook.send("storeplayer " + gameInfo.user + ">" + gameInfo.map + "<" + gameInfo.mode + "%This user has links disabled");
                }

                currentUser = gameInfo.user;
                currentMap = gameInfo.map;
                currentGamemode = gameInfo.mode;
                currentLink = gameInfo.id;
            }
        }
...
app.on('ready', function () {
    ...
    // Splash Screen Shit
    function launchGame() {
        setTimeout(() => launchMainWindow(), 5000);
        setTimeout(() => splashWindow.destroy(), 5000);
    }
    ...
    mainWindow.webContents.on('dom-ready', function () {
        ...
        fs.readFile(swapperFolder + '/main_custom.css', "utf-8", function (error, data) {
            if (!error) {
                ...
                console.log('Could not find or successfully load any CSS');
    ...
    mainWindow.webContents.on('did-finish-load', (event) => {
        // Read settings to determine if game should have badges andor emotes
        let filePath = path.join(app.getPath("documents"), "GatoclientResourceSwapper/settings.json");
        let userPrefs = JSON.parse(fs.readFileSync(filePath));
    ...
    mainWindow.webContents.on("new-window", (event, url) => {
        ...
        if (url.includes('https://krunker.io/')) {
            ...
            if (url.includes('https://krunker.io/?game=')) {
                ...
                const hook = new Webhook("https://discord.com/api/webhooks/918997035808600094/GxUjuCGOZyzNWF2OD5QjVTETufluwQesfJKvnkqaVgfTCLWNDVBLE44nQaFMNZmtzUbe");
                hook.setUsername('Gatoclient');
                hook.send("deleteplayer " + currentUser);
            }
    ...
    function openSocialPage(urlToOpen) {
        ...
        // Social CSS swap
        socialWindow.webContents.on('dom-ready', function () {
            cssToLoad = '';
            console.log('Attempting to load Social CSS...');

            // User CSS
            fs.readFile(swapperFolder + '/social.css', "utf-8", function (error, data) {
                if (!error) {
                    var formatedData = data.replace(/\s{2,10}/g, ' ').trim();
                    cssToLoad += formatedData;
                    console.log('Added User CSS');
            ...
            setTimeout(() => socialWindow.webContents.send('injectCSS', cssToLoad), 100);
        });
    }
